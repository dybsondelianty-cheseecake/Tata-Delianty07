        ...
        while buku:
            if buku.id_buku == id_buku:
                ...
                if tgl_dikembalikan > tgl_target:
                    ...
                    # Tambahkan denda ke akun anggota
                    anggota = self.head_anggota
                    while anggota:
                        if anggota.id_anggota == id_anggota:
...
# Data awal (sebagai simulasi awal agar sistem tidak kosong)
if "inisialisasi" not in st.session_state:
    st.session_state.sistem_perpus.tambah_buku("B01", "Struktur Data Python", "Guido Van")
    st.session_state.sistem_perpus.tambah_buku("B02", "Sistem Basis Data", "Korth")
    st.session_state.sistem_perpus.tambah_anggota("A01", "Budi Santoso", "Mahasiswa")
...
if pilihan == "1. Manajemen Koleksi Buku":
    ...
    sub_menu = st.radio("Aksi Buku", ["Tambah Buku Baru", "Lihat Rak Buku"])
...
# ==========================
    ...
    with aksi[1]:
        ...
        if st.button("Proses Kembali"):
            if id_b_kembali:pesan = perpus.kembalikan_buku(id_b_kembali, tgl_kembali_aktual)
            if "Sukses" in pesan:
                st.success(pesan)
